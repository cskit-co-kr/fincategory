          ...
          <div className="white-box">
            ...
            <div className="mt-5">
              <div className="">
                ...
                단 9분께 선착순 판매합니다.
                <br />
                지금 미리 구매하셔서 자리를 선점하세요. 늦으면 매진될 수
                있습니다.
              </div>
              ...
              <div className="mt-6">
                ...
                {ads1.map((ad: ads, index: number) => (
                  <div className="flex mt-2" key={index}>
                    <div>{ad.duration}</div>
                    <div className="ml-3">
                      {ad.coin.toLocaleString()} FinCoin
                    </div>
                    <div className="ml-10">
                      {status === "unauthenticated" ? (
                        <Link href="/auth/signin">[구매하기]</Link>
                      ) : activeProducts < 9 ? (
                        <button
                          onClick={() => {
...
export const getServerSideProps = async (context: any) => {
  ...
  const response2 = await fetch(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/v1/product/getProductSection1`
  );
  ...
  const response3 = await fetch(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/v1/product/getProductActive`
  );
