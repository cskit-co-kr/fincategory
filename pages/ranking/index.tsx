...
const Ranking = (props: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  const [subscribersFrom, setSubscribersFrom] = useState({
    value: props.rankLimit?.subscribers_from ? props.rankLimit?.subscribers_from : 99,
    error: false,
  });
  const [postPerFrom, setPostPerFrom] = useState({
    value: props.rankLimit?.postperday_from ? props.rankLimit?.postperday_from : 4,
    error: false,
  });
  ...
  // Data
  const doSearch = async (field: any, order: any, tagQuery: any, categories: any) => {
    setLoading(true);
    const sorting = {
      field: field,
      order: order,
    ...
    const searchData: any = {
      query: tagQuery ? `#${tagQuery.tag}` : selectedTag ? `#${selectedTag.tag}` : null,
      withDesc: false,
      category: categories?.length > 0 ? categories : selectedCategory === null ? [] : selectedCategory,
    ...
    const response2 = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/client/telegram/searchChannel`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(searchData),
    });
  ...
  // Table
  const column = router.query.column ? (router.query.column as string) : "extra_08";
  ...
  function saveRankLimit() {
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/v1/settings/setRankFilter`, {
  ...
  return (
    <>
      <NextSeo
        title={"핀카텔레 | 랭킹-텔레그램 채널/그룹정보"}
        titleTemplate={"핀카텔레 | 랭킹-텔레그램 채널/그룹정보"}
        description={`검색결과:${totalAll} 개 채널 중${
          totalAll - total
        } 개가 필터링 되어${total} 개 채널을 찾았습니다.`}
      />
      <div className='md:pt-7 bg-gray-50'>
        ...
        <div className={`border mt-5 border-gray-200 min-h-[70vh] bg-white rounded-md p-4 md:p-[30px]`}>
          <div className='flex gap-5 flex-col lg:flex-row lg:p-4 p-1 pt-0 w-full lg:items-center justify-between'>
            <div className='text-[12px] leading-5 items-center flex'>
              ...
              <div className='flex flex-wrap'>
                <div>검색결과</div>
                {selectedTag ? <div className='font-bold'>“# {selectedTag.tag}”</div> : null} :{" "}
                <div className='font-bold ml-1'>{totalAll}</div>개 채널 중{" "}
                <div className='font-bold ml-1 text-[#FF0000]'>{totalAll - total}</div>
                개가 필터링 되어 <div className='font-bold ml-1 text-[#3687E2]'>{total}</div>개 채널을 찾았습니다.
              </div>
            </div>
            <div
              className={`text-[12px] leading-5 items-center relative flex ${
                session?.user.type === 2 ? "border p-2 rounded-md select-none group border-gray-400" : ""
              }`}
            >
              ...
              {session?.user.type === 2 ? (
                <div
                  className={`absolute text-2xl bg-white flex transition-all overflow-hidden top-1/2 z-0 -translate-y-1/2 ${
                    edit ? "translate-x-full duration-300 opacity-100" : "translate-x-0 invisible opacity-0"
                ...
                <div className='ml-1 text-[#3687E2]'>
                  {session?.user.type === 2 && edit ? (
                    <InputNumber
                      ...
                      onChange={(v: any) => setSubscribersFrom({ value: v, error: false })}
                    />
                  ) : (
                ...
                <div className='ml-1 text-[#3687E2]'>
                  ...
                  {session?.user.type === 2 && edit ? (
                    <InputNumber
                      ...
                      onChange={(v: any) => setPostPerFrom({ value: v, error: false })}
                      className='w-11 defualtNumberInput'
                      size='small'
                      min={0}
          ...
          <div className='mt-4'>
            <Table
              ...
              renderEmpty={() => <div className='text-center py-10'>{t["no-search-results"]}</div>}
            ...
            >
              <Column width={(width || 0) < 768 ? 45 : 50} align='center'>
                ...
                <Cell dataKey='rank'>
                  {(rowdata) => <div className='flex w-full h-full justify-center items-center'>{rowdata.rank}</div>}
                </Cell>
              </Column>
              ...
              <Column flexGrow={2} minWidth={(width || 0) < 550 ? (width || 0) - 100 : 300}>
                <HeaderCell>
              ...
              <Column width={locale === "ko" ? 80 : 110} align='center' sortable>
                <HeaderCell className={sortColumn === "subscription" ? "font-bold text-primary" : ""}>
                  {t["subscribers"]}
                </HeaderCell>
                <Cell
                  dataKey='subscription'
                  renderCell={(a) => (
                    <div className='flex w-full h-full justify-center items-center pr-2'>{formatKoreanNumber(a)}</div>
                  )}
                />
              </Column>

              <Column width={locale === "ko" ? 90 : 120} align='center' sortable>
                <HeaderCell className={sortColumn === "increase7d" ? "font-bold text-primary" : ""}>
                  {t["increase-7d"]}
                </HeaderCell>
              ...
              <Column width={locale === "ko" ? 120 : 134} align='center' sortable>
                <HeaderCell className={sortColumn === "extra_07" ? "font-bold text-primary" : ""}>
                  {t["posts-per-month"]}
                </HeaderCell>
                <Cell dataKey='extra_07'>
                  {(rowData) => {
                    return (
                      <div className='flex w-full h-full justify-center items-center pr-2'>
                        {!!rowData.extra_06 ? <span className=''>{rowData.extra_07}</span> : <span>0</span>}
                      </div>
                    );
                  }}
                </Cell>
              ...
              <Column width={locale === "ko" ? 105 : 125} align='center' sortable>
                <HeaderCell className={sortColumn === "extra_06" ? "font-bold text-primary" : ""}>
                  {t["views-per-post"]}
                </HeaderCell>
                <Cell dataKey='extra_06'>
                  {(rowData) => {
                    return (
                      <div className='flex w-full h-full justify-center items-center pr-2'>
                        {!!rowData.extra_06 ? <span className=''>{rowData.extra_06}</span> : <span>0</span>}
                      </div>
                    );
                  }}
                </Cell>
              ...
              <Column width={locale === "ko" ? 120 : 80} align='center' sortable>
                <HeaderCell className={sortColumn === "extra_08" ? "font-bold text-primary" : ""}>
                  {t["ERR"]}
                </HeaderCell>
                <Cell dataKey='extra_08'>
                  {(rowData) => {
                    return (
                      <div className='flex w-full h-full justify-center items-center pr-2'>
                        {!!rowData.extra_08 ? (
                          <span className=''>{parseFloat(rowData.extra_08).toFixed(2)}%</span>
                        ) : (
                          <span>0.00%</span>
                        )}
                      </div>
...
export const getServerSideProps = async () => {
  ...
  const resultRank = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/v1/settings/getRankfilter`);
