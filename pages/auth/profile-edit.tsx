...
const Profile = ({ memberInfo }: any) => {
  ...
  const schema = yup.object().shape({
    ...
    email: yup
      ...
      .test("emailAvailable", "Email is already registered", (value) =>
        validateEmail(value)
      ),
  });
  const schema2 = yup.object().shape({
    oldPassword: yup.string().required(t["You can't leave this empty."]),
    password: yup
      .string()
      .min(6, t["no to min 6"])
      .max(20, "no to max 20")
  ...
  const validateEmail = async (value: string) => {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_CLIENT_API_URL}/api/member?f=checkemail&email=${value}`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
      }
  ...
  const onSubmit = async (data: any) => {
    ...
    setLoading(true);
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_CLIENT_API_URL}/api/member?f=update`,
      {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
  ...
  const onChangePassword = async (data: any) => {
    ...
    if (result.code === 200 && result.message === "Password Changed") {
      ...
      setUpdatePasswordText(t["Old Password Incorrect!"]);
    }
  };
      ...
      <div className="flex gap-4 pt-7 pb-7 md:pb-0 bg-gray-50">
        ...
        <div className="mx-auto w-full px-5 md:px-0 gap-4">
          ...
          <div className="white-box">
            ...
            {updateText !== "" && (
              <div className="bg-gray-100 p-4 text-center rounded-lg mt-4">
                {updateText}
              </div>
            )}
            <form
              onSubmit={handleSubmit(onSubmit)}
              className="mt-4 gap-4 grid border-t border-gray-200 pt-4"
            >
              <div>
          ...
          <div className="white-box mt-4">
            <div className="font-semibold">{t["비밀번호 변경"]}</div>
            {updatePasswordText !== "" && (
              <div className="bg-gray-100 p-4 text-center rounded-lg mt-4">
                {updatePasswordText}
              </div>
            ...
            >
              <div>
                <label className="block pb-1">{t["현재 비밀번호"]}</label>
                <input
                  type="password"
                  className="input_field"
                ...
                />
                <p className="text-xs text-red-600">
                  {errors2.oldPassword?.message?.toString()}
                </p>
              </div>
              <div>
                <label className="block pb-1">{t["신규 비밀번호"]}</label>
                <input
                  type="password"
                  className="input_field"
                  {...register2("password")}
              ...
              <div>
                ...
                />
                <p className="text-xs text-red-600">
                  {errors2.confirmPassword?.message?.toString()}
                </p>
              </div>
              <button className="blue-button place-self-center" type="submit">
                {loading2 && <Loader />}등록
