...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getOrderedPosts() {
    ...
    const resp = await axios.post(
      `${process.env.NEXT_PUBLIC_API_URL}/client/telegram/getDetail/${user_id}/posts`,
      {
        paginate: {
          limit: 500,
          offset: 0,
  ...
  async function getPosts() {
    const user_id = req.body.channel_id;
    const resp = await axios.post(
      `${process.env.NEXT_PUBLIC_API_URL}/client/telegram/getDetail/${user_id}/poststotalview`,
      {
        paginate: {
          limit: 1000,
          offset: 0,
    ...
    const averages = Object.values(
      data.reduce((acc: any, { date, views }: any) => {
        ...
        acc[dateString].average = Math.round(
          acc[dateString].views.reduce((a: any, b: any) => a + b) /
            acc[dateString].views.length
        );
        return acc;
      }, {})
    );
