...
function LinkPreview(metaLink: any) {
  ...
  async function getMeta() {
    const response = await fetch(
      `https://jsonlink.io/api/extract?url=${metaLink.url}`
    );
  ...
  const fetchMeta = async () => {
    const html: any = await fetch(
      `${process.env.NEXT_PUBLIC_CLIENT_API_URL}/api/meta?url=${metaLink.url}`
    );
    ...
    <div className="text-xs border-l-2 pl-2 m-3 mt-5 gap-2.5">
      <div>
        <div className="font-semibold text-primary break-all">
          <Link
            href={meta?.url !== undefined ? (meta?.url as string) : "#"}
            target="_blank"
          >
            {meta?.url}
          </Link>
      ...
      <div className="w-full">
        <img
          src={imageUrl}
          alt={meta?.title}
          className="max-w-[280px] md:max-w-[500px] max-h-[360px] mx-auto"
        />
      </div>
    </div>
  ) : (
