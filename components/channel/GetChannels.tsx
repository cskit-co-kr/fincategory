...
const GetChannels:FunctionComponent<Props> = ({channels}) => {
  ...
  return (
    <Link href={{ pathname: "/channel/"+channels.username }} className='hover:no-underline' target={'_blank'}>
      <div className='flex items-start border border-gray-200 rounded-md bg-white p-4 gap-[10px] text-black max-h-[140px]
        ...
        <div className='flex flex-col gap-2'>
          ...
          <div className='flex'>
            ...
            <p className='text-[12px] m-0 text-gray-500 ml-auto'>오늘{channels.counter.today}/누적{channels.counter.total}</p>
          </div>
        </div>
      </div>
    </Link>
